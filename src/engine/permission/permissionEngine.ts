...
import { detectSecurityThreats } from './bashSecurity.js'
 ...
 * 3. critical 安全威胁 → deny（反弹 shell/凭据窃取等，比破坏性更严格）
 * 4. high 安全威胁 → ask
 * 5. plan 模式 + 非只读 → deny
 * 6. strict 模式 + bash → ask
 * 7. 命令只读 + 无危险路径 → allow
 * 8. 写路径不在 workspace → ask
 * 9. 命中危险路径黑名单 → deny
 * 10. 默认 → allow
 */
export function checkBashCommand(
  ...
  // Step 4b: 安全威胁检测（反弹 shell、凭据窃取、持久化后门、混淆执行等）
  // ——这一层比 destructive 更严格：critical 直接 deny，不给 ask 的机会
  const threats = detectSecurityThreats(command, parsed)
  if (threats.length > 0) {
    ...
    if (critical.length > 0) {
      ...
      return {
        behavior: 'deny',
        reason: `[安全拦截] ${t.message}（类别: ${t.category}，证据: ${t.evidence}）。此类命令不允许执行；如确需放行，请切换到 bypass 模式并设置 BYPASS_PERMISSIONS=true。`,
      }
    }

    // medium 只作为 warnings 的一部分，不影响后续决策
    if (medium.length > 0) {
      warnings.push(...medium.map(t => `⚠️ ${t.message}`))
    }

    // high 威胁：无论何种模式都强制 ask（bypass 除外，但 bypass 在 Step 2 已短路）
    if (high.length > 0) {
      warnings.push(...high.map(t => `🛡️ [${t.category}] ${t.message}`))
      return {
        behavior: 'ask',
        reason: `检测到高风险操作：${high.map(t => t.message).join('；')}`,
        warnings,
      }
    }
  }
